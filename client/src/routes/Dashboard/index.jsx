	...
	useEffect(() => {
		const fetchStats = async () => {
			try {
				...
				toast.error(
					err.message === "[object Object]" ? "Error" : err.message
				);
			}
		};
	...
	return (
		<Section>
			<Switch>
				...
				{user.role === 3 && (
					<Route path={path + "/admins"} exact component={Admins} />
				)}
				...
				<Route path={path} exact>
					<Breadcrumbs
						current={Strings.dashboard[lang]}
					...
					{stats.length ? (
						<ControlledSlider items={stats} card={SlideItem} />
					) : null}
