...
const PetView = () => {
		...
		const fetchPetDetails = async () => {
			try {
				const responseData = await sendRequest(
					"https://amused-cyan-shrimp.cyclic.app/api/pet/" +
						`${petId}`
				);
	...
	if (!isLoading && loadedPet) {
		imageURL =
			"https://amused-cyan-shrimp.cyclic.app/" + `${loadedPet.file}`;
		bgURL =
			"https://amused-cyan-shrimp.cyclic.app/uploads/images/" +
